...
#
# Copied from farm_assistant/app/services/search_service.py (unchanged logic).
# OpenSearch request orchestration against the /llm_retrieve RAG endpoint.
...
logger = logging.getLogger("agentic-fa.search")
S = get_settings()
...
def build_search_payload(inp: AskIn) -> Dict[str, Any]:
    ...
    # Pull a slightly wider candidate set than the final grounded context so the
    # controller can discard weak OpenSearch hits before prompting the LLM.
